...
foreach ($orgs as $i => $org)
{
	...
	$td[$i]['Exemption.count'] = array('.', array(
		'ajax_count_url' => array('controller' => 'exemptions', 'action' => 'org', $lineId, 'list' => $this->passedArgs['list']), 
	...
	foreach($exemptionTypes as $exemptionType_id => $exemptionType_shortname)
	{
		$td[$i]['ExemptionType.id.'. $exemptionType_id.'.count'] = array('.', array(
			'ajax_count_url' => array('controller' => 'exemptions', 'action' => 'org', $lineId, $exemptionType_id, 'list' => $this->passedArgs['list']), 
			'url' => array('action' => 'view', $lineId),
		));
	}
